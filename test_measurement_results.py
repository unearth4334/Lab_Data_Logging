...
Usage:
    python test_measurement_results.py [VISA_ADDRESS] [--output-dir OUTPUT_DIR] [--channel CH1]
...
def _filter_measurement_results(
    ...
    if statistics_mode == "1" or statistics_mode.upper() == "ON":
        filtered = [
            measurement
            for measurement in parsed_results
            if channel_norm in str(measurement.get("label", "")).upper()
        ]
        return filtered, channel_norm

    print(
        "Warning: --channel filter ignored because statistics mode does not include labels."
    )
    return parsed_results, channel_norm


def test_measurement_results(visa_address=None, output_dir="captures", channel=None):
    """
    ...
    try:
        ...
        filtered_results, channel_norm = _filter_measurement_results(
            results["parsed_results"],
            results["statistics_mode"],
            channel,
        )
        ...
        if filtered_results:
            ...
            if results['statistics_mode'] == "1" or results['statistics_mode'].upper() == "ON":
                ...
                for measurement in filtered_results:
        ...
        with open(output_file, 'w') as f:
            ...
            if channel_norm:
                f.write(f"Channel Filter: {channel_norm}\n")
            f.write("\n")
            
            if filtered_results:
                f.write(f"Parsed Results ({len(filtered_results)} measurements):\n")
                f.write("-" * 40 + "\n")
        ...
        print("=" * 60)
        print(f"Statistics Mode: {results['statistics_mode']}")
        print(f"Measurements Found: {len(filtered_results)}")
        print(f"Raw Response Length: {len(results['raw_response'])} chars")
        if channel_norm:
            print(f"Channel Filter: {channel_norm}")
...
Examples:
  ...
  python test_measurement_results.py "USB0::0x0957::0x17BC::MY56310625::INSTR"
    ...
    python test_measurement_results.py --channel CH1
        """
    )
    ...
    parser.add_argument(
        "--channel",
        help="Optional channel filter for measurement labels (e.g., CH1). Only applies when statistics mode is ON."
    )
    ...
    # Run the test
    results = test_measurement_results(args.visa_address, args.output_dir, args.channel)
