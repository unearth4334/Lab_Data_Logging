...
if [[ "$response" =~ ^[Yy]$ ]]; then
    ...
    case $choice in
        ...
        5)
            ...
            if [ -n "$channels" ]; then
                cmd="python scripts/lab_cli.py run-test --board-number $board_num --label $test_label --channels $channels"
            else
                cmd="python scripts/lab_cli.py run-test --board-number $board_num --label $test_label"
            fi
